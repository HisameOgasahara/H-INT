...
/*
 * YOASOBI - Idol - one-buzzer TC275 player
 *
...
volatile unsigned int g_idol_stop_requested = 0U;
volatile unsigned int g_idol_current_event = 0U;
volatile unsigned int g_idol_volume_adc = VADC_MAX_12BIT;
...
static void play_idol(void);

__interrupt(SW2_ISR_PRIORITY) __vector_table(0)
void ISR_SW2_IDOL_STOP(void)
{
    g_idol_stop_requested = 1U;
    buzzer_off();
}
...
void idol_run(void)
{
    ...
    while (1)
    {
        ...
        {
            g_idol_stop_requested = 0U;
            g_idol_current_event = 0U;
            play_idol();
            buzzer_off();

            while (read_sw1() == 0U)
            {
                if (g_idol_stop_requested != 0U)
                {
                    buzzer_off();
                }
            }
...
static void update_buzzer_volume(void)
{
    ...
    g_idol_volume_adc = adc;

    /*
     * Keep the period fixed and vary only the high-time:
     * A0 0..4095 -> duty 0..50%, so pitch never changes.
     */
...
static unsigned int wait_midi_ticks_abortable(unsigned int midi_ticks)
{
    ...
    divisor = (unsigned long long)IDOL_BPM * (unsigned long long)IDOL_TICKS_PER_QUARTER;
...
static void play_idol(void)
{
    ...
    for (i = 0U; i < IDOL_SCORE_EVENT_COUNT; i++)
    {
        const IdolEvent *event = &g_idol_score[i];
        g_idol_current_event = i;
        ...
        if (event->frequency_hz == IDOL_REST)
        {
            buzzer_off();
        }
        else
