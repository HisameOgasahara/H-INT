...
/*
 * Detective Conan Main Theme - TC275 one-buzzer player
 *
...
volatile unsigned int g_conan_stop_requested = 0U;
volatile unsigned int g_conan_event_index = 0U;
volatile unsigned int g_conan_volume_adc = VADC_MAX_12BIT;
...
static unsigned int wait_score_ticks_abortable(unsigned int ticks_8th);
static void play_conan(void);

__interrupt(SW2_ISR_PRIORITY) __vector_table(0)
void ISR_SW2_CONAN_STOP(void)
{
    g_conan_stop_requested = 1U;
    buzzer_off();
}

void conan_main_theme_run(void)
{
    ...
    while (1)
    {
        ...
        {
            g_conan_stop_requested = 0U;
            g_conan_event_index = 0U;
            play_conan();
            buzzer_off();

            while (read_sw1() == 0U)
            {
                if (g_conan_stop_requested != 0U)
                {
                    buzzer_off();
                }
            }
...
static unsigned int wait_score_ticks_abortable(unsigned int ticks_8th)
{
    ...
    /* one 1/8-note = 60 / BPM / 2 seconds */
    numerator = (unsigned long long)STM0_FREQUENCY_HZ * 60ULL * (unsigned long long)ticks_8th;
    target_ticks = (unsigned int)(numerator / ((unsigned long long)CONAN_BPM * 2ULL));
...
static void play_conan(void)
{
    ...
    for (i = 0U; i < g_conan_score_count; i++)
    {
        const ConanEvent *event = &g_conan_score[i];

        g_conan_event_index = i;
        ...
        if (event->frequency_hz == CONAN_REST)
        {
            buzzer_off();
        }
        else
        ...
        if (wait_score_ticks_abortable((unsigned int)event->ticks_8th) == 0U)
        {
            break;
        }
    }
